...
class MMKVInstance {
    constructor(id) {
        ...
        this.setArray = (key, value) => {
            ...
            if (this.handleNullOrUndefined(key, value))
                return true;
            assert('array', value);
            let result = (0, handlers_1.handleAction)(module_1.default.setArrayObjectMMKV, key, value, this.instanceID);
            if (result) {
                if (this.isRegisterd(key)) {
                    this.ev.publish(`${key}:onwrite`, { key, value: value });
                }
        ...
        this.setObject = (key, value) => {
            ...
            if (this.handleNullOrUndefined(key, value))
                return true;
            assert('object', value);
            let result = (0, handlers_1.handleAction)(module_1.default.setObjectMMKV, key, value, this.instanceID);
            if (result) {
                if (this.isRegisterd(key)) {
                    this.ev.publish(`${key}:onwrite`, { key, value: value });
                }
        ...
        this.getObject = (key, callback) => {
            try {
                const value = (0, handlers_1.handleAction)(module_1.default.getObjectMMKV, key, this.instanceID);
                if (value === null || value === undefined) {
                    callback && callback(null, null);
                    return null;
                }
                if (this.transactions.onread['object']) {
                    const result = this.transactions.transact('object', 'onread', key, value);
         ...
         */
        this.setUniversalObject = (key, value) => {
            if (Array.isArray(value)) {
                ...
                throw new Error(`setUniversalObject only accepts objects or arrays, received ${typeof value}`);
            }
        };
        /**
         ...
         */
        this.getUniversalObject = (key, callback) => {
            try {
                ...
                if (Array.isArray(value)) {
                    if (this.transactions.onread['array']) {
                        const result = this.transactions.transact('array', 'onread', key, value);
                        if (callback)
    ...
    getObjectAsync(key) {
        return new Promise(resolve => {
            resolve(this.getObject(key));
        });
    }
    /**
     * Async version of setUniversalObject
     */
    setUniversalObjectAsync(key, value) {
        return new Promise(resolve => {
     ...
     */
    getUniversalObjectAsync(key) {
        return new Promise(resolve => {
            resolve(this.getUniversalObject(key));
        });
    }
}
